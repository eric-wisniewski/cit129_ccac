...
def get_list_of_all_types(list_in_dict): 
    ...
    for i in list_in_dict:
        for key in i:
            ...
            if i['type'] not in list_of_types:
...
def regex(list_of_titles):
    ...
    print("For the full list of titles, the word 'Engineer' shows up in",  
counter_eng, "jobs, the word 'Developer' shows up in", counter_dev, \
"jobs, and the word 'Senior' shows up in" \
,counter_sen, "jobs.")

def writeDictFile(list_dict):
    '''
    This File writed takes something known as Pickle, a dictionary dumping
    script, and uses it to store the data. Instead of the 'w' command, 'wb' is
    used due to the API being in binary. Therefore binary mode is needed for 
    a successful dump of this file. 
    '''
    file = open('githubJobs.dat', 'wb')
    for i in list_dict:
        dump(i,file)
    file.close        
